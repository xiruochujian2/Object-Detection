...
from ultralytics.nn.backbone.efficientViT import EfficientViT_M1
...
# 没有购买yolov11项目需要注释以下
from ultralytics.nn.extra_modules import Detect_Efficient, Detect_DyHead_Prune, Detect_LSCD, Detect_RSCD, Detect_TADDH
from ultralytics.nn.extra_modules.block import Faster_Block, Fusion, IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, SimFusion_4in, AdvPoolFusion, \
    PyramidPoolAgg, RepVGGBlock, RepConvN, Star_Block, Faster_Block_CGLU, MutilScaleEdgeInformationEnhance, CSPOmniKernel, SPDConv
...
def get_pruner(opt, model, example_inputs):
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, AAttn):
            ignored_layers.append(m)
        ignored_layers.append(model.model[0])
